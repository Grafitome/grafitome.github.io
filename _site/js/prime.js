StockMarketChartManager={
	...
	attachHoveringEffect:function(){
		...
		.on("mouseup",function(){
			...
			{
				StockMarketChartManager.updateChartWithData(StockMarketChartManager.data);
				d3.select(".resetAreaBtn").style('display','inline-block')
			}	
		})
		.on("mousemove",function(){
		   if(flagForPrimeToolTip){
				var xScale=StockMarketChartManager.xScaleHistoricChart;	
		...
		.on("mouseout",function(){StockMarketChartManager.hideToolTip();})
	},
	...
	updateChartWithData:function(data){
		...
		data = data.slice(initialX,lastX);
	...
	drawHLCChart:function(lineChartConfiguration){
		...
		var resetBtnHeight = 35;
		var resetBtnWidth = 70;
		var resetBtnGrouping = d3.select("#"+id).append("div")
				  ...
				  .on("mouseout",function(){
					d3.select(this).style("background",'#66A374');
					
				  })
				  .style("width",resetBtnWidth)
				  .style("height",resetBtnHeight)
				  .style('background','#66A374')
				  .style('display','none')
				  .style("z-index","999")
	...
	/*	marginLeft=marginLeft-($("body").find("#tooltipChart-stock-market").width()*.5);
		...
		if((marginLeft-$(tooTipElem).width())<150){
