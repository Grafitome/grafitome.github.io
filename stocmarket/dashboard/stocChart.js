...
(function ($) {
    ...
    //$.fn.stocCharts = function (data, xFieldName, yFieldName,styleConfg) {
        ...
        var WM = {
            attachWaterMark: function (id) {
                selectorElement.css('background-image', 'url(stoc-chart-logo.png)');
                selectorElement.css('background-repeat', ' no-repeat');
                selectorElement.css('background-position', 'bottom right');
            }
        }
        ...
        var toolTipManager = {
			...
			appendToolTip:function(){
				var tootTipTemplate ='<div id="tooltipChart" style="float:none;display:none; margin:0px; padding:0px; position:relative; width:150px;">'+
					...
					'	</div>'+
					'	<div style="position: relative; bottom:1px; height:40px; width:36px; '+
            ...
            showToolTip: function (e, yValArg, xValArg, hideXVal, yHeadingMap) {
				...
				*/
                ...
                var marginTop = e.pageY - ($("body").find("#tooltipChart").height()*1.3);
				marginLeft=marginLeft-($("body").find("#tooltipChart").width()*1.2);
        ...
        var lineChart = {
            ...
            drawBarWithMultipleLineAndCircle: function (options) {
                ...
                var xAxisElem = svgElement.append("g")
                    .attr("class", "x axis")
                    .attr("transform", "translate(" + (left) + "," + (height + options.marginTop) + ")")
                    .attr("stroke", options.axisColor)
                    .attr("fill", 'none')
                    .call(xAxis);
                ...
                var bars = svgElement1.selectAll(".bar")
                    ...
                    return "multichart bar " + d.name + " " + "pos_" + Math.floor(xScale(d.x));
                })
                    .attr("width", barWidth)
                ...
                var path = lineGroup.append("path")
                    ...
                    return "multipleLine " + d.name + " " + "pos_" + Math.floor(xScale(d.x));
                })
                ...
                lineGroup.selectAll("circle.multipleLineCircle")
                    ...
                    .append("circle")
                    .attr("class", function (d) {
                    return "multichart  multipleLineCircle " + d.name + " " + "pos_" + Math.floor(xScale(d.x));
                })
                    .attr('cx', function (d) {
                    return xScale(d.x);
                })
                ...
                mulitpleLineChartLegend.append("rect")
                    ...
                    if ((d3.select(this).attr("class")).indexOf("disable") != -1) {
                        ...
                        $(selectorElement).find(".multipleLineCircle." + d).show();

                        var selectedPath = svgElement.selectAll(".multipleLine." + d);
                        selectedPath.transition()
                            .duration(1000)
                            .ease("linear")
                            .attr("stroke-dashoffset", 0);

                        svgElement.selectAll(".bar." + d)
                            .transition()
                            .duration(1000)
                            .attr("y", function (d) {
                            return yScale(d.y);
                        ...
                        $(selectorElement).find(".multipleLineCircle." + d).hide();
		...
		var brushingChart={
			drawBrushing:function(callback){
				...
				function mousedown() {
					...
					var m = d3.mouse(this);
					brushedContainer = selectedArea.append("g").attr("transform", "translate("+0+"," +margin.top+ ")").append("rect")
				...
				function mouseup(){
					...
					if(isMouseMove){
						...
						for(var i=0;i<data1.length;i++){
							...
							if((dataStarting.getMonth() == objDate.getMonth()) && (dataStarting.getDate() == objDate.getDate()) && (dataStarting.getFullYear() == objDate.getFullYear())){
								startIndex=i;
								console.error("*** matched**");
								isStartIndexFound=true
							}
							if((dataEnding.getMonth() == objDate.getMonth()) && (dataEnding.getDate() == objDate.getDate()) && (dataEnding.getFullYear() == objDate.getFullYear())){
								endIndex=i;
						...
						if(isStartIndexFound && isLastIndexFound ){
							...
							//alert(data1.length);
								/*
								xScale.domain(d3.extent(data1, function(d,i) {
									return parseDate.parse(d[xFieldName]);
								}));
								*/
				...
				function mousemove() {
					...
					//	console.log("vakksjbshsddshj::: "+m[1]+"::  "+m[0]+":: "+width);
						dataStarting=new Date(xScale.invert(m[1]));
						dataEnding=new Date(xScale.invert(m[0]));
						console.log(new Date(xScale.invert(m[1])) +":::"+new Date(xScale.invert(m[0])));
						isMouseMove=true;
					}
        ...
        var drawBar = {
            ...
            groupedBarChart: function (options) {
                ...
                svgElement.append("g")
                    ...
                    .text(function (d, i) {
						var obj = dataset[d];
						//return formatDate(new Date(obj[xFieldName].replace(/-/g, "/")));
                ...
                stackChartLegend.append("rect")
                    ...
                    if (!svgElement.selectAll("rect."+d).empty()) {
		...
		var gradientBar = barRectangle.append("svg:defs")
			...
			*/
			bulletBar:function(cnfg){
				var marginBulletChart={left:width*0.2,right:width*0.2,bottom:height*0.2,top:height*0.2,chartSeparator:2,xScalePaddingTop:height*0.1,yScalePaddingLeft:width*0.05};
               ...
               // tempXScale.domain(keysArray).rangeRoundBands([0, baseXScale.rangeBand()]);
                ...
                var yAxisElem = svgElement.append('g')
                    .attr('class', 'y axis')
                    .attr("transform", "translate(" + (-marginBulletChart.yScalePaddingLeft) + "," + 0 + ")")
                    .attr("stroke", axisColor)
                    .attr("fill", 'none');
                ...
                var yAxisElem2 = svgElement.append('g')
                    .attr('class', 'y axis')
                    .attr("transform", "translate(" + (scaleWidth+marginBulletChart.yScalePaddingLeft) + "," +(0)+ ")")
                    .attr("stroke", axisColor)
                    .attr("fill", 'none');
			...
			barWithLogo:function(cnfg){
				...
				var marginCrickedAnalChart={left:width*0.1,right:width*0.1,bottom:height*0.05,top:height*0.05,chartSeparator:5};
        ...
        var drawCircle = {
            circleChart: function (options) {
                ...
                svgElement.selectAll("." + className)
                    ...
                    return (xScale(parseDate.parse(d[xFieldName])) + options.r/2);
                })
                    .attr("cy", function (d) {
					...
					toolTipManager.showToolTip(d3.event, "",(cfgArea.xAxisIndicationLabel +" "+d[xFieldName]), false,yHeadingValueMap);
			...
			.on("mouseout", function () {
				...
				*/
				...
				var legendGrouping=svgElement.selectAll(".pyramidLegendGrouping")
							  ...
							  .attr("transform", function(d, i) { return "translate(" + (0) + "," + (10+(i*22)) + ")" });
	...
	//				
		...
		var scaleX = d3.scale.linear()
							.domain([d3.min(data.xAxisData),d3.max(data.xAxisData)])
							.range([10,width-margin.right/2]); 
		var reverseScaleX = d3.scale.linear()
							.domain([10,width-margin.right/2])
							.range([d3.min(data.xAxisData),d3.max(data.xAxisData)]); 
									
		var scaleOfCountry = d3.scale.linear()
							.domain([minGDP,maxGDP*2])
							.range([height+margin.bottom-20,5]); 
		...
		//show grid view
		gridManager(svgSelection, height+margin.bottom, width-margin.right/2, margin.left, 5);
		...
		// svgSelection.selectAll('path').attr('fill','none');
		 svgSelection.append("g")
			...
			.call(xAxis);
		d3.select('.xAxis path').attr('fill','none')				
		...
		function showLineData()
		{
			var lineFunctionCountry1 = d3.svg.line()
								 .x(function(d,i) {return scaleX(data.xAxisData[i]); })
								 .y(function(d,i) 
								 {	
		...
		combinationalGroup.selectAll('rect')
		  ...
		  .attr("x", function(d, i){return scaleX(data.xAxisData[i])-rectangleSize/2;})
		...
		var lineFunctionCountry2 = d3.svg.line()
								 .x(function(d,i) { console.log("years="+data.xAxisData[i]); return scaleX(data.xAxisData[i]); })
		...
		combinationalGroup.selectAll('circle')
		  ...
		  .attr("cx", function(d, i){return scaleX(data.xAxisData[i]);})
		...
		function mouseOverOnLine(event,dataIndex) 
			...
			//	alert(dataIndex);
				lineIndex=0;	
		//		alert("value "+data.xAxisData[dataIndex]);
			//	var x = event.pageX-(width*.10);
			//	x=scaleX.invert(x);//reverseScaleX(x);
			//	x=parseInt(x);
		//		for(index=0;index<data.xAxisData.length;index++)
		//		{	
			//		if(x==data.xAxisData[index])
				//	{	
						selectedYear = data.xAxisData[dataIndex];
		//				break;
			//		}
		//		}
		//		x=data.xAxisData[dataIndex];
		//		lineArray[0]=scaleX(x)+margin.left;
	//			lineArray[1]=height+margin.bottom;
		//		lineArray[2]=scaleX(x)+margin.left;
		//		lineArray[3]=margin.top*2.8;
...
var yHeadingValueMap=[{"headingName":(data.labelLine1+" "+data.yLabel) ,"headingVal":data.dataOfLine2[keyName]},{"headingName":(data.labelLine1+" "+data.yLabel) ,"headingVal":data.dataOfLine1[keyName]}];
...
toolTipManager.showToolTip(event,"",(data.xAxisData[keyName]), false,yHeadingValueMap);
		...
		d3.select("#dountGroup")
			...
			var dountGroup = svgSelection.append("g")
						   .attr('id','dountGroup')
						   .attr("transform", "translate(" +2*margin.left + "," +(margin.top+5) + ")");
		...
		function showLegend()
		{
			var legendGroup = svgSelection.append("g")
										  .attr('id','legendGroup')					
										  .attr("transform", "translate(" +(2*margin.left+radius*1.5)+ "," + (margin.top+5-legendSize)+ ")");
		...
		{	
			var clickedSymbolGroup = svgSelection.append("g")
			...
			for(key in data.clickedInfo){
				...
				function showXAxisLabel()
				{
						var xAxisGroup = svgSelection.append("g")
								   .attr('id','xAxisGroup')
									 .attr("transform", "translate("+((margin.left+width-(margin.right/2))/2)+"," + (height+margin.bottom+18) + ")");
						xAxisGroup.append('text')
								   .style('font-size',1.3*textSizeOfAxis)
								   .style('font-family','calibri')
								   .style('font-style','bold')
								   //.attr('fill',"#2ca02c")
								   .text(data.xAxisLabel);
				}
				function showYAxisLabel()
				{
					var yAxisGroup = svgSelection.append("g")
								   .attr('id','yAxisGroup')
									 .attr("transform", "translate("+(margin.left-35)+"," + (height - 40) + ")");
        ...
        var drillDownChart = {
            drawDrillDownChart: function (id, barData,options) {
                ...
                function InitChart() {
                    ...
                    var xScale = d3.scale.ordinal().rangeRoundBands([cfg.leftMargin, width - cfg.rightMargin], 0.1).domain(browserData.map(function(d){
						return d["x"];
					})),
					...
					var largestStringLngth=(browserData[0]["name"].length);
					for(var charCountr=0;charCountr<browserData.length;charCountr++){
						var currentStringLngth=browserData[charCountr]["name"].length;
					...
					var xAxisTickArr=tickController.getXTickArray(0,(browserData.length),largestStringLngth, (width - cfg.rightMargin));
					...
					function drawNewBarChart(d) {
						...
						*/
                        ...
                        if ('undefined' !== typeof data) {
                            ...
                            xScale = d3.scale.ordinal().rangeRoundBands([cfg.leftMargin, width - cfg.rightMargin], 0.1).domain(data.map(function (d) {
                                return d.x;
                            }));
		...
		var darwVariationAnalysisGraph=
		{
				...
				{
						var variationAnalChart={left:width*0.15,right:width*0.15,bottom:height*0.15,top:height*0.15,chartSeparator:5,xScalePaddingTop:height*0.2,yScalePaddingLeft:width*0.1};
				...
				var titleGroup = variationMainGroup.append("g")
									.attr('class','title-group')
                                    .attr("transform", "translate(" + scaleWidth/4 + "," + (-variationAnalChart.top/1.4) + ")");
					titleGroup.append('text')
								...
								.text(title);
				var subTitleGroup = variationMainGroup.append("g")
									.attr('class','sub-Title-Group')
                                    .attr("transform", "translate(" + (scaleWidth/4+10) + "," + (-variationAnalChart.top/2.6) + ")");
					subTitleGroup.append('text')
								.attr('x',0)
								.attr('y',0)
							  .attr('font-size',titleFontSize/2)
							    .attr('font-family',fontFamily)
								.attr('fill',titleColor)
								.text(subTitle);			
		  ...
		  gridManager.init(variationMainGroup, scaleHeight, scaleWidth, variationAnalChart.lect, variationAnalChart.top);		
                ...
                var yAxisGroup = variationMainGroup.append("g")
									...
									.attr("transform","translate("+0+","+0+")")
						       	    .attr('fill','none')
									.call(yAxis);	
		        var yAxisText = variationMainGroup.append("g")
				                    .attr('class','y-axis-text')
				     				.attr("transform","translate("+(-variationAnalChart.left/2.3)+","+0+")");
					yAxisText.selectAll('text')
								...
								.attr('font-size',fontSize)
							    .attr('font-family',fontFamily)
								.attr('y',function(d,i){return yScale(i)+fontSize/2;})
		...
		//		alert(d3.min(lowValue)+"  " +d3.max(highValue));
				...
				var xAxisLabelGroup = variationMainGroup.append("g")
									.attr("class","x-axis-label-group")
									.attr("transform","translate("+scaleWidth/2.5+","+(scaleHeight+variationAnalChart.bottom)+")")
...
var rectangleGradient = rectangleGroup.append("svg:defs")
        ...
        var pieChartWithTransition = {
            drawPieChartWithTransition: function (options) {
				...
				*/
                ...
                var arcs = svg.selectAll("g.arc")
                    ...
                    .attr("stroke", "white")
                    .attr("stroke-width", 0)
                    .attr("transform", "translate(" + (outerRadius + curMarginLeft) + "," + (outerRadius + marginArc / 2) + ")")
                    .on("mouseover", function (d, i) {
				...
				legendGroup.selectAll('text')
						 ...
						 //  .attr('x',function(d, i){ return ((i*(width*.20))+20);})
						   .attr("x", function(d, i){ //if(i>3){return ((parseInt(i/4)-1)+(positionText++)*(widthOfDount*.3))+rectSize+3}else{return (i*(widthOfDount*.3))+rectSize+3};
								...
								/*
								var moveToNextLine=isMoveLegendToNextLine(data.key[i],xPos);
								
								if(moveToNextLine){
									xPos=0*(widthOfDount*.3)+textSeparator;
								}else{
									xPos=i*(widthOfDount*.3)+textSeparator;
								}
								*/
						   ...
						   .attr("y",function(d,i) { //if(i>3){return (3*(rectSize)) }else{return rectSize}   
								//var xPos=i*(widthOfDount*.3);
								/*
								var moveToNextLine=isMoveLegendToNextLine(data.key[i],xPos1);
								
								var yPos=legendWidth;
								if(moveToNextLine){
									nextLineCounter++;
									yPos=yPos+nextLineCounter*nextLineSeparator;
									xPos1=0*(widthOfDount*.3);
								}else{
									xPos1=i*(widthOfDount*.3);
								}
								return yPos;
								*/
			...
			//set legendColor
			//legendGroup.selectAll("text").style("fill",cnfg.legendTextColor);	
		}	
        ...
        var heatMapChart = {
            drawHeatMapChart: function (id, data,options) {
                ...
                var svg = svgElement.append("svg")
                    ...
                    .append("g")
                    .attr("transform", "translate(" + cfg.yAxisLabelSpacing + "," + margin.top + ")");
        ...
        var scatterPlotChart = {
            drawScatterPlotChart: function (id, data) {
                ...
                var xScale = d3.scale.linear()
                    .domain([0, maxX])
                    .range([cfg.padding, width - cfg.padding * 2]);
                ...
                var svg = svgElement.append("svg")
                    ...
                    .attr("height", height)
                    .append("g")
                    .attr("transform", "translate(" + (leftMargins + cfg.padding) + "," + topMargins + ")");
                ...
                function drawLineWithShape(svg, dataV, color, name) {
                    ...
                    if (dataV.shape == 'circle') {
                        drawCircle(svg, getFormattedDataFromDataSetForScatter(dataV), color, name);

                    } else if (dataV.shape == 'square') {
                        drawSquare(svg, getFormattedDataFromDataSetForScatter(dataV), color, name);
                    } else {

                        drawTriangle(svg, getFormattedDataFromDataSetForScatter(dataV), color, name);
                    }
                }
                ...
                function drawCircle(svg, data, color, name) {
                    ...
                    svg.selectAll("." + name + "dot")
						...
						var yHeadingValueMap=[{"headingName":name +" "+yIndicationLabel,"headingVal":d.y}];
				...
				var largestStringLngth=(xAxisArray[0].length);
				for(var charCountr=0;charCountr<xAxisArray.length;charCountr++){
					var currentStringLngth=xAxisArray[charCountr].length;
				...
				var xAxisTickArr=tickController.getXTickArray(0,(xAxisArray.length),largestStringLngth, (width - cfg.padding * 2));
                ...
                for (k = 0; k < actualData.length; k++) {
                    if (actualData[k].shape == 'square') {
                        ...
                        legend.selectAll('rect')
                            ...
                            .attr("y", 0)
                            .attr("width", (actualData.length / 300) * width - 1)
                            .attr("height", (actualData.length / 300) * width - 1)
                            .style("fill", function (d, i) {
								return d.color;
							})
                            .on("click", function (d) {
								var state = d3.selectAll("." + d.name + "dot").style("display");
								if (state == "none") {

									d3.selectAll("." + d.name + "dot").style("display", "block");
									d3.select("#" + d.name + "text").style("fill", "black");
								} else {

									var selectedPath = svgElement.selectAll("." + d.name + "line");

									d3.selectAll("." + d.name + "dot").style("display", "none");
									d3.select("#" + d.name + "text").style("fill", "grey");
								}
							});
                        ...
                        legend.selectAll('circle')
                            ...
                            .attr("y", 0)
                            .attr("cx", (actualData.length / 300) * width + 0.20 * width)
                            .attr("cy", (actualData.length / 300) * width - 5)
                            .attr("r", function (d, i) {
                        ...
                        legend.selectAll('triangle')
                            ...
                            .attr("d", d3.svg.symbol().type("triangle-up"))
                            .attr("transform", function (d, i) {
                            return "translate(" + ((actualData.length / 300) * width + 500) + "," + ((actualData.length / 300) * width - 5) + ")";
                        })
                            .style("fill", function (d, i) {
                            return d.color;
                        })
		...
		function setTextStyleAndSvgBackGround(svgElement){
			...
			allText.style("fill",textStyle["font-color"]);
			...
			//axis color
			svgElement.selectAll(".axis").selectAll("text").style("fill",textStyle["axisTextColor"]);
		...
		var threeDBarChart = {
			...
			var xScale = d3.scale.linear()
                    ...
                    .range([leftMargins, (heightOfBars.length * xSpacingInBars) + 50]);


            var yScale = d3.scale.linear()
                    .domain([0, 1.2 * maxHeight])
                    .range([height - 0.1 *height, 0]);             
			...
			xAxis.tickValues(d3.range(0, heightOfBars.length, 1));
		...
		var startX= width  - width * 0.89;
            ...
            var yAxisRef=svgContainer.append("g")
                    .attr("class", "axis")
                    .attr("transform", "translate(" + (padding + 40) + ","+ (-5)  +")")
                    .call(yAxis);	   
        ...
        var basicLineChart = {
            drawBasicLineChart: function (id, data) {
				...
				*/
                ...
                var x = d3.scale.linear().domain([0, (maxXScale)]).range([10, width]),
                    y = d3.scale.linear().domain([0, 1.5 * maxValue]).range([height, 0]);

                xAxis = d3.svg.axis().scale(x).ticks(10).orient("bottom"),
                ...
                function drawLineWithShape(svg, data, color, name) {
                    ...
                    if (data.Shape == 'circle') {
						drawCircle(svg, data.data, color, name);
					}
					else if (data.Shape == 'square') {

                        drawSquare(svg, data.data, color, name);
                    } else {

                        drawTriangle(svg, data.data, color, name);
                    }
                ...
                function drawCircle(svg, data, color, name) {
                    ...
                    aLineContainer.selectAll("." + name + "dot")
						...
						var yHeadingValueMap=[{"headingName":name,"headingVal":d}];
                ...
                legend.selectAll('rect')
                    ...
                    .attr("y", 0)
                    .attr("width", (actualData.length / 500) * width)
                    .attr("height", (12 / 500) * height)
                    .style("fill", function (d, i) {
				...
				var largestStringLngth=(xScaleTicksArray[0].length);
				for(var charCountr=0;charCountr<xScaleTicksArray.length;charCountr++){
					var currentStringLngth=xScaleTicksArray[charCountr].length;
				...
				var xAxisTickArr=tickController.getXTickArray(0,(xScaleTicksArray.length),largestStringLngth, width);
				xAxis.tickValues(xAxisTickArr);
		...
		var funnelChart={
			drawFunnelChart:function(data){
				...
				var factor=1.5;
				var factor1=0.8;
				var widthOfFunnelChart=(width/factor);
				var heightOfFunnelChart=(height/factor1);
				var options = {
					width: widthOfFunnelChart,          // In pixels; defaults to container's width (if non-zero)
					height: heightOfFunnelChart,         // In pixels; defaults to container's height (if non-zero)
				...
				chart.draw("#"+$(selectorElement).attr("id"));
				
				//alert("width after reduction "+(parseInt(width)/3) +" width "+width);
				var svg=d3.select("#"+$(selectorElement).attr("id")).select("svg");
				svg.style("margin-left",(svg.attr("width")/4));
			}
		}
		...
		var drawGauageGraph={
			gaugeGraph:function(options){
				...
				var vis=svgElement.append("svg:g")
				.attr("class","pie")	
				.attr("transform", "translate(" + centerChartDim + "," + r + ")")    
			...
			if(!redrawAreaChart){
				...
				if($(selectorElement).find(".axis").length == 0){	
					...
					if(options.xAxisIndicationLabel!=""){
						...
						var indicationLineAndTextGap=20;
						var yLablePos=margin.scale-(indicationLineAndTextGap/3);
						var yLinePos=margin.scale-indicationLineAndTextGap;
						
						xAxisElem.append("path")
						.attr('class',"axis-arrow-trianle")
						.attr("d",d3.svg.symbol().type("triangle-up").size(10))
						.attr("fill","red")
						.style('display','block')
						.attr("transform", function(d) { return "translate(" + indicationLineXPos0 + "," + yLinePos + ") rotate(-90)"; });
					...
					if(options.yAxisIndicationLabel!=""){
						var yIndicatorLineGap=10;
						var yIndicatorTextPos=-margin.areaLeft-yIndicatorLineGap;
						
						var lengthOfIndicatorLine=40;
						var yIndicatorLinePos0=(height/2)-lengthOfIndicatorLine;
						var yIndicatorLinePos1=(height/2)+lengthOfIndicatorLine
						
						var xPosition0=0-margin.areaLeft+yIndicatorLineGap;
						
						yAxisElem.append("line")
						.attr("class",'y-axis-indication-line')
						.attr('x1',xPosition0)
						.attr('y1',(yIndicatorLinePos0))
						.attr('x2',xPosition0)
						...
						yAxisElem.
						append("g")
						.attr("transform", "translate(" + yIndicatorTextPos + "," + (height/2) + ")" )
						.append("text")
						.attr("class",'y-axis-indication')
						.attr('text-anchor','middle')
						.attr("transform", "rotate(90)")
		...
		var threeDStackedBarChart = {
            drawThreeDStackedBarChart: function (id, heightOfBars, heightOfBarTwo, threeDStackedBarsData, options, colorArray,gridLineColor, textColor, xAxisArray) {
				...
				var xScale = d3.scale.linear()
						...
						.range([widthOfBars/2, (threeDStackedBarsData[0].data.length* xSpacingInBars) + widthOfBars/2]);
			...
			for(var q=0; q<elementLengthInData; q++){
				...
				for(var w=0; w<totalElementsType; w++){
					...
					var lineGraph1 = svgContainer.append("path")	
									.attr("class","lineGraphClass " + threeDStackedBarsData[w].key.replace(/ /g,''))
					...
					var lineGraph3 = svgContainer.append("path")
						...
						startY1 = yScale(currentHeight) - 5 * (w+1);						
			...
			function getSidesCordinates(x,y,width,height,sideno,heightOfBar, dataNumber){
				...
				var xAxisRef=svgContainer.append("g")
						.attr("class", "axis")
						.attr("transform", "translate("+startX+"," + (height - height * 0.05 - yScale(1.15 * maxHeight)) + ")")						
						.call(xAxis)
				...
				var yAxisRef=svgContainer.append("g")
						.attr("class", "axis")
						.attr("transform", "translate(" + (padding + 40) + ","+ (yScale(1.15 * maxHeight))  +")")
						.call(yAxis);
				/*
				...
				*/
				...
				var legend = svgContainer.append("g")
				  ...
				  var pixcelPerChar=6,widthLimit=width*0.7,textSeparator=20,nextLineSeparator=(maxHeight*0.02);
				  
				  
				 function isMoveLegendToNextLine(string,xPos,i){
					
					if(i == 0){
						return {"startPos":(width*0.05),"move":false};
					}
					else{
						var currentLegendXPos=xPos+legendWidth+(string.length*pixcelPerChar)+textSeparator+(width*0.1);
						
						if(currentLegendXPos>widthLimit){
							return {"startPos":(width*0.05),"move":true};
							//return true;
						}
					
						else{
							return {"startPos":currentLegendXPos,"move":false};
							//return false;
						}
					}
				}
				...
				var nextLineCounter=0;
				var xPos=(width*.05),xPos1=(width*.05),yPos=0;
				var positionXArray=[];
				var positionYArray=[];
				   ...
				   var CordBean={
                    ...
                    legend.selectAll('rect')
							.data(threeDStackedBarsData)
                            .enter()
                            .append("rect")
                            .attr("x", function (d, i) {
								/*
								var curVal=CordBean.getX();
								var newXCord2=	curVal+(d.key.length* 6)+35;
								CordBean.setX(newXCord2);
								return curVal;
								*/
                    ...
                    newXCord=0;
					nextLineCounter=0;
					xPos=0*(width*.3)+textSeparator,xPos1=0*(width*.3);
                    legend.selectAll('text')
                            .data(threeDStackedBarsData)
                            .enter()
                            .append("text")
		...
		var globAnalysisGraph = 
		{
			globChartAnalysis:function(cnfg)
			{
				...
				globMainGroup.append('text')
							 ...
							 .attr('x',(width/2)-((title.length*titleSize)/2))
							 .attr('y',titleSize)
...
StockMarketChartManager={
	...
	drawHLCChart:function(lineChartConfiguration){
		...
		var highFieldName=lineChartConfiguration.heighFieldName;
